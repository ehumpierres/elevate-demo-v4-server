...
from components.common import get_common_css, add_navigation_header
...
add_navigation_header("AI Assistant")
...
# Create a two-column layout for main content and sidebar
...
with col2:
    ...
    if st.session_state.user_id is None:
        ...
        if st.button("Start Session", use_container_width=True) and user_id:
        ...
        # Warm start button
        if st.button("Warm Start", use_container_width=True) and user_id:
            st.session_state.user_id = user_id
            st.session_state.companion = Companion(user_id, analyst_type=st.session_state.selected_analyst)
            st.session_state.llm_api = LlmApi()  # Initialize LLM API for follow-up questions
        ...
        # Train model button
        if st.button("🎓 Train SQL Model", use_container_width=True):
            with st.spinner("Training... This may take several minutes."):
                try:
                    ...
                    if not connection_test.get("success", False):
                        st.error(f"❌ Connection failed: {connection_test.get('error', 'Unknown error')}")
                        progress_bar.empty()
                        status_text.empty()
                    else:
...
with col1:
    ...
    if st.session_state.user_id:
        ...
        st.subheader("Welcome to the AI Assistant")
        st.write("Please enter your username in the settings panel to get started.")
        
        st.markdown("""
        ### What can I help you with?
        
        - **Strategic Analysis**: Get insights on GTM health, revenue architecture, and business metrics
        - **Data Analysis**: Query your Snowflake database with natural language
        - **Recommendations**: Receive actionable advice based on your data and business context
        - **Follow-up Questions**: Get smart suggestions for deeper analysis
        
        ### Available Analysts:
        
        **🏗️ Arabella (Business Architect)**
        - Revenue architecture and GTM optimization expert
        - Focuses on alignment between sales, marketing, and customer success
        - Provides mathematical rigor for business decisions
        
        **📈 Sales Motion Strategy Agent**
        - Specialized in sales process optimization
        - Pipeline analysis and conversion rate improvements
